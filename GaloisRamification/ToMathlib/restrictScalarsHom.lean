...
def AlgEquiv.restrictScalarsHom (R S A : Type*) [CommSemiring R] [CommSemiring S] [Semiring A] [Algebra R S] [Algebra S A]
...
: @IsScalarTower S F A ((reverse_algebra_if_rank_eq_one h).toSMul) _ _ :=
  by
    letI smul_instance := (reverse_algebra_if_rank_eq_one h).toSMul
